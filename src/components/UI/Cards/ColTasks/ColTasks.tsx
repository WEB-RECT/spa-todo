...
const ColTasks: FC<IProps> = ({
    ...
    const tasks = useMemo(() => {
        const filteredItems = tasksItems.filter(
            (it) =>
                it.colTasksUid === item.uid &&
                it.projectUid === currentParamsUid,
        );
        ...
        if (valueFind) {
            result = result.filter(
                (it) =>
                    it.id.toString().includes(valueFind) ||
                    it.name.includes(valueFind),
            );
        } else {
                ...
                {tasks.length > 0 ? (
                    ...
                    <>
                        {!collectedProps.isOver &&
                            collectedProps.isOverColUid === item.uid && (
                                <Typography.Text>Нету задач</Typography.Text>
                            )}
                    </>
                )}
                {collectedProps.isOver &&
                    collectedProps.canDrop &&
                    collectedProps.isOverColUid === item.uid && <CardPreview />}
            </div>
        </div>
    );
};
